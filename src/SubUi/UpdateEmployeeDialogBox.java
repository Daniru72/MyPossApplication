...
public class UpdateEmployeeDialogBox extends javax.swing.JDialog {

   String url = "/resourse/eye1.png";
   
   private static String mobile;
   
    public UpdateEmployeeDialogBox(java.awt.Frame parent, boolean modal, String mobile) {
        super(parent, modal);
        initComponents();
        this.mobile = mobile;
     ...
     private void loadUserType() {
      ...
      private void loadData() {
        ...
        try {
            ...
            ResultSet resultSet = MySQL.execute("SELECT * FROM `user` INNER JOIN `gender` ON `user`.`gender_gender_id`=`gender`.`gender_id` "
                                                  + " INNER JOIN `status` ON `user`.`status_status_id`=`status`.`status_id` "
                                                    + " INNER JOIN `type` ON `user`.`type_type_id`=`type`.`type_id` WHERE `mobile`='"+mobile+"'");
            ...
            if (resultSet.next()) {
                ...
                jTextField4.setText(resultSet.getString("username"));
                jTextField5.setText(resultSet.getString("user_email"));
                jPasswordField4.setText(resultSet.getString("password"));
                jComboBox2.setSelectedItem(resultSet.getString("type.type_name"));
    ...
    private void initComponents() {
        ...
        setPreferredSize(new java.awt.Dimension(525, 677));
        setResizable(false);

        kGradientPanel1.setkEndColor(new java.awt.Color(0, 4, 126));
        kGradientPanel1.setMaximumSize(new java.awt.Dimension(525, 677));
        kGradientPanel1.setMinimumSize(new java.awt.Dimension(525, 677));
        kGradientPanel1.setPreferredSize(new java.awt.Dimension(525, 677));
    ...
    private void kButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_kButton1ActionPerformed
        ...
        String genderId = buttonGroup2.getSelection().getActionCommand();
            String statusId = buttonGroup1.getSelection().getActionCommand();

        
        
            try {
                
                 MySQL.execute("UPDATE `user` SET `fname`='" + fname + "', "
                        + "`lname`='" + lname + "', "
                                 + "`username`='" + username + "', "
                                          + "`password`='" + Password + "', "
                        + "`type_type_id`='" + userTypeMap.get(userType) + "', "
                        + "`gender_gender_id`='" + genderId + "',"
                        + "`status_status_id`='" + statusId + "' WHERE `mobile`='"+mobile+"' ");
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                UpdateEmployeeDialogBox dialog = new UpdateEmployeeDialogBox(new javax.swing.JFrame(), true, mobile);
